  ...
  <div class="myDetail">
    ...
    <view class="flex-col page">
      <view class="flex-col group">
              ...
              <view class="flex-row justify-between group_3">
                <view class="flex-row space-x-22">
                  ...
                  <view class="flex-col space-y-26 group_4">
                    <view class="flex-row">
                      <text
                        style="font-size: 32rpx; font-family: PingFang-SC-Medium; line-height: 26rpx; color: #333333; font-weight: 500"
                        class="text"
                        >{{ userInfo.name }}
                      </text>
                      <!-- vip图标 -->
                    ...
                    <view class="flex-row space-x-14">
                      ...
                      <text class="font_1 text_3">{{ userInfo.introduction }}</text>
                    </view>
                  </view>
                </view>
        ...
        <!-- 主要内容 -->
        <div class="content-wrapper" v-for="(item, index) in single_text" :key="index">
          <!-- 头像 -->
          <UserHeader
            v-if="userInfo"
            :time="item.update_at"
            ...
            :name="userInfo.name"
            :isfootprint="userInfo.is_footprint"
          >
          </UserHeader>
          <!-- 内容 -->
          <ArticleContent
            :textContent="item.single_text_content"
            :isContentAll="true"
            :imgUrlAry="item.media"
            :itemData="item"
          ></ArticleContent>
          <Comments
            class="my-comments"
            :type_forward="item.type_forward"
            :type_comments="item.type_comments"
            :type_likes="item.type_likes"
            :is_like="item.is_like"
            :single_text_id="item.single_text_id"
            @sub="sub"
            @zf="zf(item)"
          ></Comments>
        </div>
        <!-- <CommunityInfoCard v-for="item in 5" :key="item" :iShowForward="item % 2 == 0" :iShowFollow="false"></CommunityInfoCard> -->
      </view>
    </view>
...
export default {
  name: 'myDetail',
  components: { UserHeader, IsFootprint, ArticleContent, Comments, CommentsInput },
  props: {},
  data() {
    return {
      user_id: '',
  ...
  methods: {
    ...
    async getUserSingleText() {
      await this.$api.user
        ...
        .then(res => {
          this.page++
          this.single_text.push(...res.data.single_text)
        })
        .catch(err => {})
    },
    /** 评论组件中传来的点击事件
