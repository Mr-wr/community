        ...
        <view class="flex-col section">
          <view class="flex-row items-center justify-between section_2">
            ...
            <view @tap="clickSearch" class="flex-col items-center text-wrapper">
              <text class="font_1 text_2">搜索</text>
            </view>
          </view>
        </view>
      </view>
      <div style="height: 80rpx; background: #f2f2f2">
        <div
          style="
          ...
          "
        >
          {{ searchData.length > 0 ? '搜索结果' : '最近联系人' }}
        </div>
      </div>
      <view v-if="searchData.length > 0" class="my-recommend">
        <RecommendList
          @clickHeader="clickHeader"
          :iShowIsFootprint="false"
          :itemData="item"
          v-for="(item, index) in searchData"
          :key="index"
        ></RecommendList>
      </view>
      <view v-else class="my-recommend">
        <RecommendList
          @clickHeader="clickHeader"
          :iShowIsFootprint="false"
          :itemData="item"
...
export default {
  ...
  methods: {
    ...
    clickHeader() {
      ...
      this.$store.commit('user/SET_USER_RELEASE_SELECT_USER', true)
      this.publicBack()
    },
    // 网络获取-经常访问
